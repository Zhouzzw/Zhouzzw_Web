...
- 简介区布局（A4，≥1024）：12 栅格「文字占 1–6 列 + 照片占 8–12 列右偏」，第 7 列留作呼吸空隙；照片 4:3、`object-position: 20% 35%`（<1024 单列堆叠，照片满宽）
- 项目卡布局（A6，≥1024）：`.project-item` 三列 `132px | 1fr | 1fr` —— 编号列 + **描述左列 + 成果右列并排**（成果段挂 `.project-item__section--results` 占第 3 列）；标题 / 标签 / 图片 / 视频段仍跨 `2 / -1`。`--results` 的 `grid-column` 须写在 `.project-item__section` 基类**之后**（同特异性靠顺序取胜）；<1024 为 flex 竖排，`grid-column` 不参与
...
4. **页面高度依赖视口高度，跨 `--h` 不比较** —— `.hero` 用 `100svh`（桌面）/ `90svh`（移动），视口高 900 与 844 会让整页差 56px。回归比对必须**锁定同一 `--h`**，且只信「同一轮 run 内 base↔cur」的差值，跨版本 / 跨参数的绝对值一律不可比。
   当前高度基准（CDP 实测）：**1440×900＝14257px · 390×844＝14332px · 360×844＝14555px**（桌面 15640→14257 系 A4/A6 布局重排所致，2026-09-16）。
