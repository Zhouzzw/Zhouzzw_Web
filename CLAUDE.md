...
- CTA 双药丸按钮动效（`.cta-split`，2026-09-16 借鉴 CodeBuddy 推广按钮动效、配色仍用站点橙）：动效语言 = **对角跃迁（左下 → 右上）**；箭头字符恒为 `↗`（**全程指向右上、不旋转、无平移中间态** —— 首版「自左平移滑入 + 旋转 45°」经用户看效果否决）。**hover** = 橙色底块 `__right::before` 以 `transform-origin: left bottom` + `scale(0→1)` 自左下角生长铺满至右上；箭头 `ctaArrowJump` 动画（静止时偏左下 `translate(-4px,4px)` → 左下淡出 → 换位 → 右上 `translate(4px,-4px)` 淡入）、同时转 `--color-black-deep`。**按下** = 底块保持点亮 + 箭头 `ctaArrowDash`（沿 ↗ 方向飞出、自左下飞回）+ 按钮下压归位（hover 的 −2px 让位）。hover 走 `@media (hover:hover) and (pointer:fine)` 门控（触屏靠 `:active` 同款点亮），减动效只留静态色（箭头 `transform: none` 归中）。箭头字符包在内层 `.cta-split__arrow` 中，勿把色变直接写在 `__right` 上
